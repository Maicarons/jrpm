...
set(CPACK_PACKAGE_VENDOR "OpenTTD (JRPM)")
set(CPACK_PACKAGE_DESCRIPTION "OpenTTD (JRPM)")
set(CPACK_PACKAGE_DESCRIPTION_SUMMARY "OpenTTD (JRPM)")
set(CPACK_PACKAGE_HOMEPAGE_URL "https://github.com/Maicarons/jrpm")
set(CPACK_PACKAGE_CONTACT "https://github.com/Maicarons/jrpm")
set(CPACK_PACKAGE_INSTALL_DIRECTORY "OpenTTD-JRPM")
...
# When building a release via CI, the authoritative version is passed in as
# JRPM_RELEASE_VERSION (the full tag name, e.g. "jrpm-0.2.0"). This must come
# BEFORE include(CPack), because include(CPack) snapshots CPACK_PACKAGE_VERSION
# / CPACK_PACKAGE_FILE_NAME into CPackConfig.cmake at the moment it runs. It
# also overrides the CPackProperties.cmake generated by FindVersion, whose
# `git describe` cannot see the real tag inside the detached source tarball used
# by platform jobs (it previously resolved to the wrong tag, e.g. jrpm-0.1.0
# instead of the released jrpm-0.1.3). The tag name itself already carries the
# "jrpm-" prefix, which matches the historical binary naming (openttd-jrpm-x.y.z),
# so it is used verbatim -- do NOT strip the prefix here.
if(DEFINED JRPM_RELEASE_VERSION AND JRPM_RELEASE_VERSION STREQUAL "")
    # ignore empty override
elseif(DEFINED JRPM_RELEASE_VERSION)
    set(CPACK_PACKAGE_VERSION "${JRPM_RELEASE_VERSION}")
    string(REPLACE "#CPACK_PACKAGE_VERSION#" "${JRPM_RELEASE_VERSION}" CPACK_PACKAGE_FILE_NAME "${CPACK_PACKAGE_FILE_NAME}")
    message(STATUS "Release version overridden by CI: ${JRPM_RELEASE_VERSION}")
endif()
