...
# JRPM_RELEASE_VERSION (the tag name with any prefix stripped, e.g. "0.2.0").
# This must come BEFORE include(CPack), because include(CPack) snapshots
# CPACK_PACKAGE_VERSION / CPACK_PACKAGE_FILE_NAME into CPackConfig.cmake at the
# moment it runs. It also overrides the CPackProperties.cmake generated by
# FindVersion, whose `git describe` cannot see the real tag inside the detached
# source tarball used by platform jobs (it previously resolved to the wrong tag,
# e.g. jrpm-0.1.0 instead of the released jrpm-0.1.3).
...
elseif(DEFINED JRPM_RELEASE_VERSION)
    # Strip any "jrpm-" tag prefix so the binary version is the bare x.y.z.
    string(REGEX REPLACE "^jrpm-" "" _jrpm_release_version "${JRPM_RELEASE_VERSION}")
    set(CPACK_PACKAGE_VERSION "${_jrpm_release_version}")
    string(REPLACE "#CPACK_PACKAGE_VERSION#" "${_jrpm_release_version}" CPACK_PACKAGE_FILE_NAME "${CPACK_PACKAGE_FILE_NAME}")
    message(STATUS "Release version overridden by CI: ${_jrpm_release_version}")
endif()
